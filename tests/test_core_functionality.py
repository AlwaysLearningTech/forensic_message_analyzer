...
class TestCoreComponents:
    ...
    def test_forensic_recorder(self, tmp_path):
        ...
        hash_value = recorder.compute_hash(temp_file)
        assert hash_value is not None
        assert len(hash_value) == 64  # SHA-256 hash

    def test_forensic_integrity(self, tmp_path):
        ...
        dest_dir.mkdir()
        working_copy = integrity.create_working_copy(source_file, dest_dir)
        assert working_copy.exists()
        assert working_copy.read_text() == "Original content"

    def test_data_extractor(self, tmp_path):
    ...
    def test_threat_analyzer(self, tmp_path):
    ...
    def test_sentiment_analyzer(self, tmp_path):
    ...
    def test_behavioral_analyzer(self, tmp_path):
    ...
    def test_pattern_analyzer(self, tmp_path):
        ...
        patterns_path = tmp_path / "test_patterns.yaml"
    ...
    def test_manual_review_manager(self, tmp_path):
    ...
    def test_timeline_generator(self, tmp_path):
    ...
    def test_run_manifest(self, tmp_path):
        ...
        output_file.write_text("Output file")

        # Add files to manifest
        manifest.add_input_file(input_file)
        manifest.add_output_file(output_file)
    ...
    def test_communication_metrics(self, tmp_path):
        """Test communication metrics analysis."""
        recorder = ForensicRecorder(tmp_path)
        analyzer = CommunicationMetricsAnalyzer(forensic_recorder=recorder)
